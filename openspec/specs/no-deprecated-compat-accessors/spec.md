# no-deprecated-compat-accessors Specification

## Purpose
TBD - created by archiving change remove-deprecated-accessors. Update Purpose after archive.
## Requirements
...
- **THEN** 模块与类型均不存在（#350 P9：无已验证端点的恒 `Err` + setter 死值 stub 删除；改用 `doc_wiki`/`schema`/`app`/`message`/`data_source`）
