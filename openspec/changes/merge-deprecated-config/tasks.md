...
- [ ] T3: core `Config::from_env()` / `load_from_env()` 从 client 迁移，适配 Arc<ConfigInner> 封装；保留 `OPENLARK_*` 环境变量语义；`OPENLARK_TIMEOUT` → `req_timeout(Some(Duration))`（分叉 5）；内部调 validate 不阻塞
