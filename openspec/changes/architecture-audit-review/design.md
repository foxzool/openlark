...
**Goals:**
- 消除 ServiceRegistry 的"伪注册"问题，使其成为真正的服务容器或完全移除
- 统一 Config 体系，减少一层间接
...
### Decision 4: HTTP 中间层 — 延迟到 v0.18

**选择**: 在 design 中规划 tower 中间件引入，但作为独立 change 延迟实施。
...
**选择**: 制定统一规范 —— 所有业务 crate 导出 `XxxClient` struct，不使用 type alias 或 Arc 包装。

**理由**:
- 一致性降低学习成本
- struct 允许未来添加方法而不破坏 API
...
1. **v0.17.0**: Config deprecated、根 crate 清理、命名统一、testing feature 修复、`async_fn_in_trait` 移除、`docs.rs` metadata
2. **v0.18.0**: ServiceRegistry 宏改造、tower 中间件层（可选）
3. **v1.0.0**: 移除所有 deprecated API，锁定公开 API
