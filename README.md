...
```toml
[dependencies]
openlark = { version = "0.17.0", default-features = false, features = ["auth"] }
```
...
**推荐业务开发**：
```toml
openlark = { version = "0.17.0", default-features = false, features = ["auth", "communication"] }
```

**按需选择**：
```toml
# 文档协作：多维表格 + Sheets helper
openlark = { version = "0.17.0", default-features = false, features = ["auth", "docs-bitable"] }

# 云盘上传/文件夹遍历
openlark = { version = "0.17.0", default-features = false, features = ["auth", "docs-drive"] }

# 自定义机器人卡片 + 签名
openlark = { version = "0.17.0", default-features = false, features = ["webhook-card", "webhook-signature"] }

# 通讯 + 文档组合
openlark = { version = "0.17.0", default-features = false, features = ["auth", "communication", "docs-bitable"] }

# 文档全量能力
openlark = { version = "0.17.0", default-features = false, features = ["auth", "docs-full"] }
```
