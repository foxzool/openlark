//! 订阅审批实例状态变更事件（v4）
//!
//! docPath: <https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/subscription>
...
/// Body/Response（参考同域 `instance` 资源其它 leaf）。
pub struct SubscribeInstanceRequestV4 {
    config: Arc<Config>,
}

impl SubscribeInstanceRequestV4 {
    /// 创建请求实例。
    ...
    pub async fn execute_with_options(
        ...
        let endpoint = ApprovalApiV4::InstanceSubscribe;
...
mod tests {
    ...
    #[test]
    fn test_subscribe_instance_v4_url() {
        let endpoint = ApprovalApiV4::InstanceSubscribe;
        assert_eq!(
            endpoint.to_url(),
    ...
    /// 端到端：POST /open-apis/approval/v4/instances/subscription，断言 path + body 透传。
    #[tokio::test]
    async fn test_subscribe_instance_returns_data_on_success() {
        ...
        SubscribeInstanceRequestV4::new(config)
