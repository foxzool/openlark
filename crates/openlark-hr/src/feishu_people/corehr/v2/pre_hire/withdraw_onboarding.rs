...
impl WithdrawOnboardingRequest {
    ...
    pub async fn execute_with_options(
        ...
        Transport::request_typed(request, &self.config, Some(option), "撤销入职响应数据为空").await
    }
}
...
mod tests {
    ...
    /// 端到端：POST /open-apis/corehr/v2/pre_hires/withdraw_onboarding
    #[tokio::test]
    async fn test_withdraw_onboarding_returns_data_on_success() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/open-apis/corehr/v2/pre_hires/withdraw_onboarding"))
        ...
        WithdrawOnboardingRequest::new(config)
            .execute()
            .await
            .expect("请求应成功");
