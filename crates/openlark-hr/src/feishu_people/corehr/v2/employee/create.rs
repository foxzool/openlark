...
impl CreateRequestBuilder {
    ...
    pub async fn execute_with_options(
        ...
        // 发送请求
        Transport::request_typed(request, &self.config, Some(option), "添加人员响应数据为空").await
    }
}
...
mod tests {
    ...
    /// 端到端：POST /open-apis/corehr/v2/employees
    #[tokio::test]
    async fn test_create_returns_data_on_success() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/open-apis/corehr/v2/employees"))
        ...
        CreateRequestBuilder::new(config)
