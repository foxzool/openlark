...
impl WithdrawRequest {
    ...
    pub async fn execute_with_options(
        ...
        Transport::request_typed(request, &self.config, Some(option), "撤销转正响应数据为空").await
    }
}
...
mod tests {
    ...
    /// 端到端：POST /open-apis/corehr/v2/probation/withdraw
    #[tokio::test]
    async fn test_withdraw_returns_data_on_success() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/open-apis/corehr/v2/probation/withdraw"))
