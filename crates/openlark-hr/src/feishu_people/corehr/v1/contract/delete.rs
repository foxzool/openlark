...
impl DeleteRequest {
    ...
    pub async fn execute_with_options(
        ...
        // 4. 发送请求
        Transport::request_typed(request, &self.config, Some(option), "删除合同响应数据为空").await
    }
}
...
mod tests {
    ...
    /// 端到端：DELETE /open-apis/corehr/v1/contracts/test001
    #[tokio::test]
    async fn test_delete_returns_data_on_success() {
        let server = MockServer::start().await;
        Mock::given(method("DELETE"))
            .and(path("/open-apis/corehr/v1/contracts/test001"))
