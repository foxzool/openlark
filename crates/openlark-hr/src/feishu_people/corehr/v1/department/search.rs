...
impl SearchRequest {
    ...
    pub async fn execute_with_options(
        ...
        // 4. 发送请求
        Transport::request_typed(request, &self.config, Some(option), "搜索部门响应数据为空").await
    }
}
...
mod tests {
    ...
    /// 端到端：POST /open-apis/corehr/v1/departments/search
    #[tokio::test]
    async fn test_search_returns_data_on_success() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/open-apis/corehr/v1/departments/search"))
