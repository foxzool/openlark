...
mod tests {
    ...
    /// 端到端：GET /open-apis/corehr/v1/leaves/leave_balances
    #[tokio::test]
    async fn test_leave_balances_returns_data_on_success() {
        let server = MockServer::start().await;
        Mock::given(method("GET"))
            .and(path("/open-apis/corehr/v1/leaves/leave_balances"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "code": 0, "msg": "success", "data": { "leave_balances": [], "has_more": false }
            })))
            .mount(&server)
            .await;
        ...
        LeaveBalancesRequest::new(config)
