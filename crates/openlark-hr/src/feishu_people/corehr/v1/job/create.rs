...
impl CreateRequest {
    ...
    pub async fn execute_with_options(
        ...
        // 4. 发送请求
        Transport::request_typed(request, &self.config, Some(option), "创建职务响应数据为空").await
    }
}
...
mod tests {
    ...
    /// 端到端：POST /open-apis/corehr/v1/jobs
    #[tokio::test]
    async fn test_create_returns_data_on_success() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/open-apis/corehr/v1/jobs"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "code": 0, "msg": "success", "data": { "job_id": "" }
            })))
            .mount(&server)
            .await;
