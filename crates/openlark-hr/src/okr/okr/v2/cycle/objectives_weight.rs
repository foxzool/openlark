...
impl Request {
    ...
    ) -> SDKResult<UpdateCycleObjectivesWeightResponse> {
    ...
    pub async fn execute_with_options(
        ...
        let req: ApiRequest<UpdateCycleObjectivesWeightResponse> =
            ApiRequest::put(path).body(body_val);
        Transport::request_typed(req, &self.config, Some(option), "修改 OKR 目标权重").await
    }
}

/// 修改 OKR 目标权重响应。
#[derive(Debug, Clone, Deserialize)]
pub struct UpdateCycleObjectivesWeightResponse {
    /// 目标列表。
    #[serde(default)]
    pub items: Option<Vec<Objective>>,
}

impl ApiResponseTrait for UpdateCycleObjectivesWeightResponse {
...
mod tests {
    ...
    #[test]
    fn test_update_cycle_objectives_weight_response_deserialize() {
        ...
        let resp: UpdateCycleObjectivesWeightResponse =
