...
/// 获取用户维度统计数据的请求构建器。
pub struct ListAdminUserStatRequestBuilder {
    start_date: String,
    end_date: String,
...
impl ListAdminUserStatRequestBuilder {
    /// 创建新的请求构建器。
...
/// 旧名兼容别名（将在 v1.0 移除）
#[deprecated(note = "renamed to ListAdminUserStatRequestBuilder, will be removed in v1.0 (#271)")]
pub type ListAdminUserStatBuilder = ListAdminUserStatRequestBuilder;
...
mod tests {
    ...
    fn test_builder_basic() {
        let config = openlark_core::config::Config::builder()
            ...
            .build();
        let request = ListAdminUserStatRequestBuilder::new(config.clone())
