...
**Goal:** 把 `openlark-security` crate 的 acs 子模块（31 个端点 + 6 个资源 Service）改造成 SDK 唯一规范实现——走 `openlark_core::Config`（**owned，非 `Arc`**）+ `Transport::request` + `validate_required!`，删除绕过 Transport 的原始 `reqwest::Client::new()` Service/Builder 层，并修复 `rule_external` 的字段语义错误。

## ⚠️ 执行状态（2026-06-21 更新，接力者必读）

**已完成（分支 `refactor/acs-transport-migration`）：**
- ✅ Task 1（边界转换）— commit `70165541e`
- ✅ Task 2（users 组**样板**）— commit `ec48d07ac`，80 tests pass，clippy/fmt 干净

**样板验证后纠正的 3 个计划错误（以下文为准，旧步骤已过时）：**

1. **`Config` 是 owned，不是 `Arc<Config>`**。`openlark_core::Config` 内部已 `Arc<ConfigInner>`，clone 廉价。communication/auth crate 惯例都是 owned `Config`。样板 `user/get.rs` 已用 `config: Config`。
2. **31 个端点文件此前从未编译**——它们在单数目录（`user/`、`device/` 等）下，**没有任何 `mod.rs` 声明过**，是纯 `validate_apis.py` 覆盖率填充物。所以"已挂 Transport"是错的；每个端点都要从零写，且要**新建 `<singular>/mod.rs` 声明子模块**（见样板 `user/mod.rs`、`user/face/mod.rs`），并在 `v1/mod.rs` 声明 `pub mod <singular>;`。
3. **`R`（`ApiRequest<R>` 泛型）是响应 `data` 字段的内容类型，不是包装层**。core 的 `Transport::request` 把 JSON 解析为 `Response<R> = {code,msg,data: R,...}`，`resp.data: Option<R>`。所以响应 struct **不要**写成 `XxxResponse { data: Option<...> }`（会导致双重嵌套）。正确做法：`R` 直接是数据内容——无 schema 的用 `serde_json::Value`，execute 返回 `Option<R>` 解包后的值（见样板 `user/get.rs` 返回 `serde_json::Value`、`user/face/get.rs` 返回 typed `FaceData`）。`R` 上 `impl ApiResponseTrait { fn data_format() -> ResponseFormat::Data }`。

**接力的正确起点：读 commit `ec48d07ac` 的 `user/get.rs` + `users/mod.rs` + `v1/mod.rs` 作为唯一可信模板。** 下方各 task 的代码片段是早期草稿，遇到与样板冲突时**以样板为准**。
...
1. **现状分类**（已验证）：31 个端点文件在单数目录（`user/`、`device/`…）下，**从未被任何 `mod.rs` 声明、从未编译**——是纯覆盖率填充（见上方"执行状态"纠正点 2）。它们虽写了 `Transport::request` + `Arc<Config>` 的壳，但：
   - **全部缺** `validate_required!` 和 `.with_supported_access_token_types([App])`。
   - **多数**返回 `Ok(XxxResponse { data: None })` 丢弃响应；有的路径/方法错（如 `user/create.rs` 用 `GET` + 假 `.replace()` 链）。
   - **结论**：不能信任任何现有端点文件，每个都要从零重写，并新建 `<singular>/mod.rs` + 在 `v1/mod.rs` 声明模块。
2. **Service/Builder 层**（`users/mod.rs` 等 6 个复数 `mod.rs`）用原始 `reqwest::Client::new()` 手工发请求，是真正被 `AcsV1Service` 调用的路径，但**字段语义也错**（rule_external）。本计划把真实逻辑迁到端点文件，把复数 `mod.rs` 瘦化为门面。Task 2 已示范（`users/mod.rs`、`user_faces/mod.rs`）。
3. **Token**：`ApiRequest` 默认 `supported_access_token_types = [User, Tenant]`，但 acs 是应用级接口，**必须显式设 `[AccessTokenType::App]`**（`.with_supported_access_token_types(vec![AccessTokenType::App])`）。这是每个端点都要做的动作。
4. **Config 构造**：`Config::builder().app_id(..).app_secret(..).base_url(..).build()` 是规范构造链。Service/Request 持 **owned `Config`**（非 `Arc`，见纠正点 1）。
...
**修改：**
- `crates/openlark-security/src/lib.rs` — `SecurityServices::new` 加 SecurityConfig→Config 转换 ✅ Task 1
- `crates/openlark-security/src/acs/acs/mod.rs` — `AcsProject`/`AcsV1Service` 改吃 owned `Config` ✅ Task 1
- `crates/openlark-security/src/acs/acs/v1/mod.rs` — 声明单数端点目录（`pub mod user;` 等）+ 复数门面目录 ✅ Task 2（users/user_faces 已接，其余待 Task 3-5）
- `crates/openlark-security/src/acs/acs/v1/<singular>/mod.rs` — **新建**，声明端点子模块 ✅ Task 2（user/ 已建，其余待建）
- `crates/openlark-security/src/acs/acs/v1/{users,devices,visitors,user_faces,access_records,rule_external}/mod.rs` — 瘦化为门面 Service（删 Builder/reqwest）✅ users/user_faces 已做
- 31 个端点 `.rs` 文件 — 从零重写为规范模式（7/31 done in Task 2）

**不动：** `security_and_compliance/**`、`models/SecurityConfig`、`models/mod.rs`。

---

## Task 1: 边界转换 — SecurityServices 接 core Config ✅ 已完成（commit `70165541e`）

> 接力者跳过此 task。实现见 `acs/acs/mod.rs`（`AcsProject`/`AcsV1Service` 吃 owned `Config`）+ `lib.rs`（`SecurityServices::new` 做 SecurityConfig→Config 转换 shim）。下方旧步骤（`Arc<Config>`）已过时，保留仅作历史记录。
...
## Task 2: users 资源组重写（user/get, list, patch, create, delete + face 子组）✅ 已完成（commit `ec48d07ac`）

> 接力者**把此 task 当作唯一可信模板**。读 `user/get.rs`、`user/list.rs`、`user/face/get.rs`、`users/mod.rs`、`user_faces/mod.rs`、`v1/mod.rs` 理解规范模式，然后对 Task 3-5 复制。下方旧代码片段是早期草稿，与样板冲突时以样板为准。
...
> **接力起点。** 完全照抄 Task 2 样板（`user/get.rs` 模式）：
> 1. 在 `v1/mod.rs` 确认 `pub mod device;` `pub mod client_device;` 已声明（Task 2 已加，目前是占位 `mod.rs`）。
> 2. 新建 `device/mod.rs`（声明 7 个子模块）+ `client_device/mod.rs`（声明 `get`）。
> 3. 重写 8 个端点 `.rs`：owned `Config` + `ApiRequest<R>`（R = `serde_json::Value`）+ `.with_supported_access_token_types(vec![App])` + `validate_required!`（路径参数）+ `Transport::request` + 返回 `resp.data.ok_or_else(...)`。
> 4. 重写 `devices/mod.rs` 为门面（持 `Config`，方法返回 `*Request::new`）。
> 5. 在 `AcsV1Service` 加 `devices` 字段 + 访问器（照抄 users/user_faces 的接法）。
