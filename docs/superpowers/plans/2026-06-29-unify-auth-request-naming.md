...
> ⚠️ **PIVOT（build 阶段，权威，覆盖下文所有「→ XxxRequest」代码块）**：实证发现 5/13 目标 `XxxRequest` 名与 `crate::models` 的请求体 body 模型 `XxxRequest` 撞名（E0255）。**新规则：所有 builder 重命名为 `XxxRequestBuilder`**（body 模型保持 `XxxRequest` 不动）。即下文每处「`XxxBuilder` → `XxxRequest`」均改为「`XxxBuilder` → `XxxRequestBuilder`」；alias 写作 `pub type XxxBuilder = XxxRequestBuilder;`；`TenantAccessTokenInternalRequestBuilder` 已是目标形式（不动，但仍可加确认）。re-export / service 方法 / 测试同步用 `XxxRequestBuilder`。此规则零撞名、对齐 helpdesk。下文代码块为「→ Request」旧版，**以本 PIVOT 为准**。
...
- [x] **Step 1: 复跑核实命令，确认 13 个 struct 仍为 `pub struct` 且位置未变**
...
- [x] **Step 2: 确认 AuthorizationUrlBuilder 存在且本次不动**
...
- [x] **Step 3: 确认 examples/tests/docs 无外部引用**
...
- [x] **Step 4: 确认根 crate 仅命名空间 re-export**
...
- [x] **Step 1: 单类型重命名 AppAccessTokenBuilder → AppAccessTokenRequest**
...
- [x] **Step 2: 其余 4 个 v3 类型同理重命名 + 加 alias**
...
- [x] **Step 3: 同步 auth/auth/v3/auth/mod.rs 的 5 个 pub use + AuthServiceV3 5 方法**
...
- [x] **Step 4: 同步 auth/mod.rs:13-16 的 5 个 pub use**
...
- [x] **Step 5: 增量验证（仅 auth crate，快速反馈）**
...
- [x] **Step 1: 5 个 create.rs/get.rs 各自重命名 struct+impl+文件内测试，并加 alias**
...
- [x] **Step 2: 同步 oidc/mod.rs（2 pub use + OidcService 2 方法）**
...
- [x] **Step 3: 同步 authen/v1/mod.rs（4 pub use + AuthenServiceV1 2 方法）**
...
- [x] **Step 4: 同步 access_token/mod.rs、refresh_access_token/mod.rs、user_info/mod.rs 各 1 个 pub use**
...
- [x] **Step 5: 同步 authen/mod.rs:12-13 顶层 pub use**
...
- [x] **Step 1: AuthorizationBuilder → AuthorizationRequest（定义文件 + alias）**
...
- [x] **Step 2: AuthorizationBuilder 3 层 re-export 链同步**
...
- [x] **Step 3: IdentityCreateBuilder → IdentityCreateRequest（定义 + alias + 1 re-export）**
...
- [x] **Step 4: VerificationGetBuilder → VerificationGetRequest（仅定义 + alias，无 re-export）**
...
- [x] **Step 1: 追加旧名 alias 可调用测试（`#[allow(deprecated)]` 抑制 warning）**
...
- [x] **Step 1: 在 `### Breaking Changes` 段顶部追加（v0.18 软 breaking，alias 兼容）**
...
- [x] **Step 1: 全 feature 构建**

Run: `cargo build --workspace --all-features`
Expected: exit 0。

- [x] **Step 2: 三组 feature clippy（default / all-features / no-default-features，均 -D warnings）**
...
- [x] **Step 4: alias deprecation warning 实证（临时代码，验证后删除）**
...
- [x] **Step 5: grep 确认改名完整性 + AuthorizationUrlBuilder 未动**
