...
- [x] **Step 3: 新建 `common/models.rs`（9 struct 整块迁移）**
...
- [x] **Step 4: 在 `v2/mod.rs` 声明 `pub mod common;`**
...
- [x] **Step 5: build 验证（中间状态：common 已建，叶子仍用 inline）**
...
- [x] **Step 2: 改 `cycle/objective/list.rs`**
...
- [x] **Step 3: 改 `cycle/objectives_position.rs`**

同模式。删 `Objective` + `ObjectiveOwner`，加 import，保留 Response wrapper。

- [x] **Step 4: 改 `cycle/objectives_weight.rs`**
...
- [x] **Step 1: 改 `indicator/patch.rs`（canonical 叶）**
...
- [x] **Step 2: 改 `objective/indicator/list.rs`**

同模式。删 3 个 Indicator struct，加 import，保留 list Response wrapper。

- [x] **Step 3: 改 `key_result/indicator/list.rs`**
...
- [x] **Step 1: 改 `key_result/get.rs`（canonical）**
...
- [x] **Step 2: 改 `key_result/patch.rs`**
...
- [x] **Step 3: 改 `alignment/get.rs`（canonical）**
...
- [x] **Step 4: 改 `objective/alignment/list.rs`**
...
- [x] **Step 3: grep 单一定义（spec Scenario "9 跨叶共享 struct 各只定义一次"）**
...
- [x] **Step 4: 确认 11 叶显式具名 import（spec Scenario "11 叶子 import 引用共享定义"）**
...
- [x] **Step 5: 确认 per-leaf Response wrapper 仍 inline（spec Scenario "per-leaf Response wrapper 保持 inline"）**
...
- [x] **Step 6: byte-identical 抽样（spec Scenario "行为零变化" 的定义一致性）**
...
- [x] **Step 7: 跨 crate 回归（spec Scenario "行为零变化" 的外部消费）**
...
- [x] **Step 8: 全量 workspace fmt/clippy 兜底（可选但推荐）**
