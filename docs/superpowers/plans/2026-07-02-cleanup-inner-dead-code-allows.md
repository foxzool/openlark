...
- [x] **Step 1: 复核无 crate 启用 tracing-init/otel**
...
- [x] **Step 2: 复核 hr/docs 仅启用 testing（保留的 feature）**
...
- [x] **Step 3: 复核无测试代码引用 otel/tracing-init feature 符号**
...
- [x] **Step 4: 删 4 个 `[dependencies]` 行 + 4 个 `[dependencies.X]` 表**
...
- [x] **Step 5: 删 3 个 `add_headers` 测试用例**
...
- [x] **Step 1: 删 `endpoints/` 目录**

```bash
rm -rf crates/openlark-hr/src/endpoints
```

- [x] **Step 2: 删 lib.rs 的注释 + allow + mod 声明（L67-69）**
...
- [x] **Step 3: 验证 hr 编译 + testing feature 仍工作**
...
- [x] **Step 2: `User.config` 加 `#[expect(dead_code)]` + 注释**
...
- [x] **Step 3: 验证两 crate 编译 0 dead_code**
...
- [x] **Step 1: 清空 `KNOWN_INNER_DEBT` heredoc（L6-17）**
...
- [x] **Step 1: fmt check（CI lint 第一步）**
...
- [x] **Step 2: clippy 三组 feature × `--all-targets` 0 dead_code**
...
- [x] **Step 5: 同步 `.github/msrv/Cargo.lock`（删依赖必做）**
...
- [x] **Step 6: 更新 CHANGELOG v0.18 Breaking Changes 区**
...
- [x] **Step 7: commit msrv lockfile + CHANGELOG**
