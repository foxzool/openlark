...
- [ ] **Step 1.1：删 11 个普通业务 crate 的 reqwest 行**
...
- [ ] **Step 1.1b：移除 12 个 crate 的 cargo-machete ignored 列表中的 `"reqwest"` 项**
...
- [ ] **Step 1.2：删 auth crate 的 reqwest（依赖行 + oauth feature 项）**
...
- [ ] **Step 2.3：验证脚本——清理后通过（happy path）**
...
- [ ] **Step 4.1：插入「Transport HTTP 边界」小节**
...
- [ ] **Step 4.2：验证 grep 能定位关键词（覆盖 3 个文档化 Requirement）**
...
- [ ] **Step 5.1：send.rs `shared_client()` 注释追加交叉引用**
...
- [ ] **Step 5.2：client.rs `WebhookClient` 注释追加交叉引用**
...
- [ ] **Step 7.1：全 feature 构建**

Run: `cargo build --workspace --all-features`
Expected: exit 0（覆盖 Scenario「全 feature 构建通过」）。

- [ ] **Step 7.2：三组 feature clippy（均 -D warnings）**
...
- [ ] **Step 7.6：守卫脚本存在性与可执行（覆盖 Scenario「守卫脚本存在」）**
